...
# DES SIR со сценарием вакцинации.
...
# DES SIR с детерминированной длительностью болезни.
function simulate_sir_des_fixed_recovery(
    ...
    # Для начальных инфицированных заранее задаём фиксированное время выздоровления.
...
# Формирование общей таблицы сравнения.
...
# Сохранение таблиц.
CSV.write(datadir("sir_des_analysis_des.csv"), df_des)
CSV.write(datadir("sir_des_analysis_ode.csv"), df_ode)
CSV.write(datadir("sir_des_analysis_vaccination.csv"), df_vaccination)
CSV.write(datadir("sir_des_analysis_fixed_recovery.csv"), df_fixed)
CSV.write(datadir("sir_des_analysis_fixed_recovery_events.csv"), df_fixed_events)
CSV.write(datadir("sir_des_analysis_comparison.csv"), df_comparison)
CSV.write(datadir("sir_des_analysis_performance.csv"), df_performance)
...
# График 1. Сравнение DES и ODE по инфицированным.
...
savefig(p_compare_infected, plotsdir("sir_des_analysis_des_vs_ode.png"))

# График 2. Сравнение базового сценария и вакцинации.
p_vaccination = plot(
    df_des.t,
    df_des.I,
    label = "Base infected",
...
savefig(p_vaccination, plotsdir("sir_des_analysis_vaccination.png"))

# График 3. Сравнение стохастического и фиксированного выздоровления.
p_fixed = plot(
    df_des.t,
    df_des.I,
...
savefig(p_fixed, plotsdir("sir_des_analysis_fixed_recovery.png"))

# График 4. Сравнение пика инфекции.
...
savefig(p_peak_comparison, plotsdir("sir_des_analysis_peak_comparison.png"))

# График 5. Сравнение итогового размера эпидемии.
p_final_size = bar(
    x_positions,
...
savefig(p_final_size, plotsdir("sir_des_analysis_final_size_comparison.png"))

# График 6. Производительность при разных размерах популяции.
p_performance = plot(
...
savefig(p_performance, plotsdir("sir_des_analysis_performance.png"))

println()
println("SIR DES analysis completed.")
println("Saved data:")
println("  data/sir_des_analysis_des.csv")
println("  data/sir_des_analysis_ode.csv")
println("  data/sir_des_analysis_vaccination.csv")
println("  data/sir_des_analysis_fixed_recovery.csv")
println("  data/sir_des_analysis_fixed_recovery_events.csv")
println("  data/sir_des_analysis_comparison.csv")
println("  data/sir_des_analysis_performance.csv")
println("Saved plots:")
println("  plots/sir_des_analysis_des_vs_ode.png")
println("  plots/sir_des_analysis_vaccination.png")
println("  plots/sir_des_analysis_fixed_recovery.png")
println("  plots/sir_des_analysis_peak_comparison.png")
println("  plots/sir_des_analysis_final_size_comparison.png")
println("  plots/sir_des_analysis_performance.png")
