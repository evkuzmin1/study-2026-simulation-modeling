...
# ----------------------------------------------------------------------
# График 3: скорости изменения популяций
# ----------------------------------------------------------------------
plt3 = plot(
    df_lv.t,
...
# ----------------------------------------------------------------------
# График 4: относительные изменения
# ----------------------------------------------------------------------
...
if length(spectrum_prey) > 1
    ...
    println("\nДоминирующая частота колебаний жертв: ", round(dominant_freq_prey, digits = 4))
...
if !isnan(peak_time_prey) && !isnan(peak_time_predator)
    ...
    println("\nАнализ колебаний:")
    println(
        "Первый пик жертв: время = ", round(peak_time_prey, digits = 2),
        ", значение = ", round(peak_value_prey, digits = 2)
    )
    println(
        "Первый пик хищников: время = ", round(peak_time_predator, digits = 2),
...
function analyze_parameter_sensitivity(param_index, values, param_name)
    ...
    for val in values
        ...
        println(
            " $(param_name)=$(val): жертвы=$(round(prey_end, digits = 2)), ",
